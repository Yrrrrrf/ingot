...
class SceneView(QWidget):
    ...
    def __init__(self, scene_manager: SceneManager = None):
        ...
        self._position = QPointF(
            0.0, 0.0
        )  # Camera position (center of viewport in scene coordinates)
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if middle_button or (left_button and alt_modifier):
            ...
            scene_pos = self._screen_to_scene(
                QPointF(event.position().x(), event.position().y())
            )

            # Check if any scene items were clicked
            for item in reversed(
                self._scene_manager.get_items()
            ):  # Check topmost items first
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if self._is_panning:
            ...
            scene_delta = QPointF(
                -screen_delta.x() / self._zoom, -screen_delta.y() / self._zoom
            )
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if event.key() == Qt.Key.Key_Left:
            ...
            self._zoom_to_cursor(
                ...
                if self._last_mouse_pos != QPointF(0, 0)
    ...
    def _zoom_to_cursor(self, cursor_pos: QPointF, zoom_factor: float):
        ...
        self._position = QPointF(
            old_position.x() - delta_scene.x(), old_position.y() - delta_scene.y()
        )

    def _screen_to_scene_with_zoom(
        self, screen_pos: QPointF, zoom_level: float
    ) -> QPointF:
        """
    ...
    def _reset_zoom(self):
        ...
        self._zoom_to_cursor(
            center_pos, 1.0 / self._zoom
        )  # Calculate factor to get back to 1.0
    ...
    def _apply_camera_transform(self, painter):
        ...
        painter.translate(-self._position.x(), -self._position.y())
